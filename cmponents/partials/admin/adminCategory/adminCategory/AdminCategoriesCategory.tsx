...
export const AdminCategoriesCategory = ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(`/api/categories/${category.categorySlug}`, {
        method: "PUT",
        headers: {
    ...
    <div onClick={(e) => e.stopPropagation()}>
      ...
      >
        <form onClick={(e) => e.stopPropagation()} onSubmit={handleSubmit} className="post-form">
          <label className="post-form-label">CategoryName: {category.categoryName}</label>
          <input
          ...
          />
          <label className="post-form-label">CategorySlug: {category.categorySlug}</label>
          <input
