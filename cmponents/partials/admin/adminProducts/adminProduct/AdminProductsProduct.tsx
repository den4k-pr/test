        ...
        <div onClick={() => handleToggle(0)} className={`admin__posts-post-form ${burgerToggle === index + 1 ? "admin__posts-post-formActive" : ""}`}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={handleSubmit} className="post-form">
            ...
            <div className="post-form-boxImg" key={idx}>
              {image && <img src={image} alt={`Image ${idx}`} />}
              <input
                name={`image-${idx}`}
            ...
            <label className="post-form-label">Name: {product.name}</label>
            <input onChange={(e) => setNewName(e.target.value)} name="name" className="post-form-input" type="text" placeholder="name" />
            <label className="post-form-label">Description: {product.description}</label>
            <textarea onChange={(e) => setNewDescription(e.target.value)} name="description" className="post-form-input" placeholder="description" />
            <label className="post-form-label">Price: {product.price}</label>
            <input onChange={(e) => setNewPrice(Number(e.target.value))} name="price" className="post-form-input" type="number" placeholder="price" />
            <label className="post-form-label">Slug: {product.slug}</label>
            <input onChange={(e) => setNewSlug(e.target.value)} name="slug" className="post-form-input" type="text" placeholder="slug" />
            <label className="post-form-label">Category: {product.category}</label>
